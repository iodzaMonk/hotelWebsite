  ...
  <div id="popup" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="space-y-1">
          <h2 class="text-2xl font-semibold tracking-tight"><?= $modal['title'] ?>
          </h2>
          <p class="text-gray-300 text-sm"><?= $modal['desc'] ?>
          </p>
        </div>
      </div>
...
<div
  class="relative bg-gray-800/40 rounded-2xl p-4 flex flex-col justify-center items-center mb-10 border border-white/30 overflow-hidden shadow-2xl shadow-white/2">
  ...
  <form action="<?= Configuration::get('rootWeb','/') ?>AdminHotel/delete/<?= $hotel['id'] ?>" method="post">
    <button type="submit" class="group cursor-pointer absolute inset-0 h-1/2 w-[50px] my-auto left-9/10 text-red-500">
      ...
      <span
        class=" opacity-0 scale-50 bg-red-500 z-20 blur-md group-hover:opacity-40 group-hover:scale-100 transition-all duration-700 ease-out" />
    </button>
  </form>
</div>
      ...
      <?php foreach ($roomsList as $room): ?>
        <div class="group block">
          <div
            ...
            <h2 class="text-xl font-semibold"><?= $room['room_type'] ?></h2>
